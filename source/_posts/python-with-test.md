...
Array 클래스의 `sum`은 아무 일도 하지 않고 `0`을 리턴하기 때문에 우리가 기대하는 대로 동작하지 않고 있다. Function을 아래와 같이 수정한 뒤 다시 테스트해 보았다.
...
기존의 `test_sum`에서는 테스트가 성공하였지만 마지막 문제를 검증하기 위한 `test_sum_raise_exception`에서는 테스트가 실패한 것을 볼 수 있다. 이대로라면 `sum` Function은 올바르지 않은 Argument가 전달되고 정상적으로 작동되는 가장 무서운 결과를 초래할 수도 있어 보인다.
...
행복한 일이 벌어졌다. 지금까지 테스트 코드를 먼저 작성하는 것으로 파이썬에 적응해 나아가는 모습을 살펴보았다. 모든 내용을 정리하지는 못했지만 이 과정에서 파이썬의 Built-in Keyword 부터 Function, Classes, Variables를 표현하는 연습도 저절로 할 수 있었다.

파이썬을 A부터 Z까지 학습한 뒤 프로젝트에 활용할 수도 있겠지만 TDD를 통해 다양한 문제에 노출되면서 주도적으로 학습에 필요한 내용을 찾아보았다. 예제는 아주 간단했지만 앞으로 우리가 작성하는 코드는 점점 거대해지며 나중에는 `레거시`라는 괴물로 변하기도 한다.
